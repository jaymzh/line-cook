...
import json
import logging
from typing import List, Optional, Tuple, Dict, Union, cast, Any
...
def load_config() -> Dict:
    ...
    """
    config_path = Path("line-cook.yml")
    default_config = {
        "bot_label": "line-cook",
        ...
        for key, value in user_config.items():
            if key == "upstream_overrides" and isinstance(value, dict):
                ...
                config["upstream_overrides"] = {
                    **cast(
                        Dict[str, Any], default_config["upstream_overrides"]
                    ),
                    **value,
                }
            else:
...
def _validate_config(config: Dict) -> None:
    ...
    if "prefix" in primary:
        # Normalize: strip trailing underscores
        normalized = primary["prefix"].rstrip("_")
        primary["prefix"] = normalized
    ...
    if universe:
        for name, upstream_config in universe.items():
            ...
            # Normalize: strip trailing underscores
            normalized = upstream_config["prefix"].rstrip("_")
            upstream_config["prefix"] = normalized
            prefixes.append(normalized)
...
class LineCook:
    ...
    def __init__(
        ...
        fix_missing_baselines: bool = False,
        base_branch: Optional[str] = None,
        pr_branch_prefix: Optional[str] = None,
        target_remote: str = "origin",
    ):
        self.config = config
        self.dry_run = dry_run
        self.force_bootstrap = force_bootstrap
        self.fix_missing_baselines = fix_missing_baselines

        self.base_branch = (
            base_branch
            if base_branch is not None
            else config.get("base_branch", "main")
        )
        self.pr_branch_prefix = (
            pr_branch_prefix
            if pr_branch_prefix is not None
            else config.get("pr_branch_prefix", "line-cook")
        )
        ...
        # Conflict details storage for exception handling
        self._last_conflict_details: Optional[str] = None
    ...
    def _setup_upstreams(self) -> None:
        ...
        primary_prefix = primary_config.get("prefix", "fb")
        ...
        # Remote name is prefix-based with underscore
        primary_remote = f"{primary_prefix}_upstream"
        ...
        for name, upstream_config in universe_upstreams.items():
            ...
            # Remote name uses prefix with underscore
            remote = f"{prefix}_upstream"
            branch = upstream_config.get("branch", "main")

            # Trailer key uses prefix with underscore
            trailer_key = f"{prefix}_Upstream-Commit"
    ...
    def _check_labels_exist(self) -> None:
        ...
        """
        bot_label = self.config["bot_label"]
        split_label = self.config["split_label"]
        ...
        try:
            ...
            if missing_labels:
                labels_str = "', '".join(
                    [label for label in missing_labels if label]
                )
                raise RuntimeError(
    ...
    def _determine_upstream_from_pr(self, pr: Dict) -> Optional[Dict]:
        ...
        for prefix, upstream_config in self.upstreams.items():
            ...
            # Match trailer at beginning of line or after whitespace
            pattern = rf"(^|\s){re.escape(trailer_key)}:"
    ...
    def _get_upstream_for_cookbook(self, cookbook: str) -> Optional[Dict]:
        ...
        """
        for prefix, upstream_config in self.upstreams.items():
            if cookbook.startswith(f"{prefix}_"):
                return upstream_config
        return None
    ...
    def _extract_upstream_commits(
        ...
        Returns:
            List of commit SHAs if return_all=True,
            single commit SHA or None if return_all=False
        """
        cmd = [
            "log",
            branch,
    ...
    def existing_sync_pr(self, upstream_config: Dict) -> Optional[Dict]:
        ...
        """
        prefix = upstream_config["prefix"]
        # Branch pattern: {pr_branch_prefix}/{prefix}_/update
        branch_prefix = f"{self.pr_branch_prefix}/{prefix}_"
        ...
        cmd = self._build_gh_pr_command(
            ...
            state="open",
            json_fields="number,headRefName,labels",
        )
        output = run(cmd)
    ...
    def get_branch_trailers(
        ...
        # When return_all=True, result is always a list
    ...
    def pr_title_and_description_from_commits(
        ...
        prefix = self.config["bot_command_prefix"]
        cmds = [
    ...
    def _build_gh_pr_command(
        ...
        Args:
            action: "create", "edit", "list", "view", "comment"
            title: PR title (for create/edit)
            body: PR body (for create/edit) or comment body (for comment)
            **kwargs: Additional options depending on action:
                - For "create": branch, base
                - For "edit": pr_number, add_label (label to add)
                - For "list": base, state, json_fields
                - For "view": pr_number, json_fields
                - For "comment": pr_number
        ...
        if action == "edit":
            ...
            if "add_label" in kwargs:
                cmd.extend(["--add-label", kwargs["add_label"]])
            elif title or body:
                # Add bot label only when setting title/body
                bot_label = self.config["bot_label"]
    ...
    def add_comment(
        ...
        if not self.dry_run:
            ...
            if is_issue:
                ...
                cmd = self._build_gh_pr_command(
                    "comment", body=body, pr_number=pr_number
                )
            run(cmd)
        else:
            self.logger.info(
    ...
    def existing_onboarding_pr(self, prefix: str) -> Optional[Dict]:
        """
        Check for an existing open onboarding PR for a specific upstream.
        ...
        Returns:
            PR dict or None
        """
        branch = f"{self.pr_branch_prefix}/{prefix}_onboard"

        self.logger.debug(f"Searching for existing onboarding PR for {prefix}")
        cmd = self._build_gh_pr_command(
            "list",
            base=self.base_branch,
            state="open",
        ...
        for pr in prs:
            if pr["headRefName"] == branch:
                self.logger.debug(
                    f"Found existing onboarding PR for {prefix}: #{pr['number']}"
                )
                return pr
        self.logger.debug(f"No existing onboarding PR found for {prefix}")
        return None

    def create_onboarding_pr(
        self, baseline: str, trailer_key: str, prefix: str
    ) -> Optional[int]:
        """
        Create or update onboarding PR to establish initial upstream sync
        baseline.
        ...
        existing_pr = self.existing_onboarding_pr(prefix)
        if existing_pr:
            pr_number = existing_pr["number"]
            self.logger.info(
                f"Updating existing onboarding PR #{pr_number} with baseline: {baseline}"
            )
        else:
            self.logger.info(
                f"Creating onboarding PR with baseline: {baseline}"
            )
...
"""
            ...
            if existing_pr:
                # Update existing PR
                self.logger.info(f"Updating PR #{pr_number} body")
                cmd = self._build_gh_pr_command(
                    "edit", title, body, pr_number=pr_number
                )
                run(cmd)
                self.logger.info(
                    f"Onboarding PR #{pr_number} updated with new baseline"
                )
                return pr_number
            else:
                # Create new PR
                ...
                pr_number = int(pr_url.strip().split("/")[-1])
                self.logger.info(
                    f"Onboarding PR #{pr_number} created: {pr_url}"
                )
                return pr_number
        else:
            action = "Updated" if existing_pr else "Created"
            self.logger.info(f"[dry-run] {action} onboarding branch {branch}")
            return None
    ...
    def is_commit_already_applied(
        ...
        try:
            ...
            for file_path in files_in_commit:
                if file_path.startswith(f"cookbooks/{prefix}_"):
        ...
        for file_path in conflicting_files:
            if file_path.startswith(f"cookbooks/{prefix}_"):
                ...
                if len(parts) >= 2:
                    cookbook = parts[1]
                    if (
                        cookbook.startswith(f"{prefix}_")
        ...
        if not success:
            ...
            try:
                ...
                if auto_resolve and not real_conflicts:
                    ...
                    self._abort_cherry_pick_safely()
                    # Store conflict info for the caller to access
                    self._last_conflict_details = conflict_info
                    raise RuntimeError(f"Conflict while applying {commit}")

            except RuntimeError:
                # Store basic conflict info if not already set
                if not hasattr(self, "_last_conflict_details"):
                    self._last_conflict_details = basic_conflict_info
                raise
            except Exception as e:
    ...
    def get_current_pointer(self, upstream_config: Dict) -> Optional[str]:
        ...
        # Pattern matches: start of line, optional whitespace, optional bullet,
        # optional whitespace, then the trailer key
        log = git(
            "log",
            self.base_branch,
            f"--grep=^\\s*\\*\\?\\s*{trailer_key}:",
            "-n",
            "1",
            "--pretty=format:%B",
        )
    ...
    def detect_global_baseline(
        self, upstream_config: Dict
    ) -> Tuple[Optional[str], List[str]]:
        """
        Detect the baseline commit for an upstream by checking all local
        cookbooks.
        ...
        Returns:
            Tuple of (baseline commit SHA or None, list of cookbooks
            without baselines)
        """
        self.logger.info(
        ...
        if missing_cookbooks:
            self.logger.debug(
                f"{len(missing_cookbooks)} cookbooks without baselines: "
                f"{', '.join(missing_cookbooks)}"
            )
        return base, missing_cookbooks
    ...
    def get_cookbooks_missing_baselines(
        self, upstream_config: Dict
    ) -> List[str]:
        """
        Get list of cookbooks that don't have a baseline in upstream.

        Args:
            upstream_config: Upstream configuration dict

        Returns:
            List of cookbook names without baselines
        """
        cookbooks = self.list_local_cookbooks(upstream_config)
        missing = []
    ...
    def sync_cookbook_to_baseline(
        self, cookbook: str, baseline: str, upstream_config: Dict
    ) -> bool:
        """
        Sync a cookbook to match upstream at baseline commit.
        ...
        Returns:
            True if successful, False if cookbook not in upstream
        """
        self.logger.info(f"Syncing {cookbook} to baseline {baseline[:8]}")

        # Get the cookbook content from upstream at baseline
        remote = upstream_config["remote"]
        upstream_tree = f"{baseline}:cookbooks/{cookbook}"

        # Check if cookbook exists in upstream at this commit
        ok, _, _ = try_git("cat-file", "-e", upstream_tree)
        if not ok:
            self.logger.warning(
                f"Cookbook {cookbook} not found in upstream at {baseline[:8]}"
            )
            return False

        # Remove existing cookbook to handle deletions properly
        cookbook_path = f"cookbooks/{cookbook}"
        self.logger.debug(f"Removing existing {cookbook}")
        ok, _, _ = try_git("rm", "-rf", cookbook_path)
        if not ok:
            self.logger.debug(f"No existing {cookbook} to remove")

        # Extract cookbook from upstream at baseline
        self.logger.debug(f"Extracting {cookbook} from {baseline[:8]}")
        try:
            git("checkout", baseline, "--", cookbook_path)
        except RuntimeError as e:
            self.logger.error(f"Failed to checkout {cookbook}: {e}")
            return False
        ...
        ok, _, _ = try_git("diff", "--cached", "--quiet")
        if ok:
            self.logger.info(f"No changes needed for {cookbook}")
            return True

        # Create commit
        trailer_key = upstream_config["trailer_key"]
        msg = f"""Sync {cookbook} to upstream baseline
...
"""
        try:
            git("commit", "-sm", msg)
            self.logger.info(
                f"Successfully synced {cookbook} to {baseline[:8]}"
            )
            return True
        except RuntimeError as e:
            self.logger.error(f"Failed to commit {cookbook}: {e}")
            return False

    def existing_fixup_pr(self, prefix: str) -> Optional[Dict]:
        """
        Check for existing open fix-up PR for missing baselines.
        ...
        Returns:
            PR dict or None
        """
        branch = f"{self.pr_branch_prefix}/{prefix}_fix_missing_baselines"

        self.logger.debug(f"Searching for existing fix-up PR for {prefix}")
        cmd = self._build_gh_pr_command(
            "list",
            base=self.base_branch,
            state="open",
        ...
        for pr in prs:
            if pr["headRefName"] == branch:
                self.logger.debug(
                    f"Found existing fix-up PR for {prefix}: #{pr['number']}"
                )
                return pr
        self.logger.debug(f"No existing fix-up PR found for {prefix}")
        return None

    def create_or_update_fixup_pr(
        self,
        missing_cookbooks: List[str],
        baseline: str,
        upstream_config: Dict,
    ) -> Optional[int]:
        """
        Create or update PR to fix cookbooks without baselines.

        Args:
            missing_cookbooks: List of cookbooks to fix
            baseline: Baseline commit SHA to sync to
            upstream_config: Upstream configuration dict
        ...
        existing_pr = self.existing_fixup_pr(prefix)
        if existing_pr:
            pr_number = existing_pr["number"]
            self.logger.info(f"Updating existing fix-up PR #{pr_number}")
        else:
            self.logger.info(
                f"Creating fix-up PR for {len(missing_cookbooks)} cookbooks"
            )
        ...
        for cb in missing_cookbooks:
            success = self.sync_cookbook_to_baseline(
                cb, baseline, upstream_config
            )
            if success:
                synced.append(cb)
            else:
        ...
        if not synced and not failed:
            self.logger.warning("No changes to commit for fix-up")
            return None

        if not synced:
            self.logger.warning(f"All {len(failed)} cookbooks failed to sync")
            # Still create/update PR to document the failure
        else:
            self.logger.info(
                f"Successfully synced {len(synced)} of "
                f"{len(missing_cookbooks)} cookbooks"
            )

        # Check if any commits were actually made
        base_sha = git("rev-parse", self.base_branch)
        current_sha = git("rev-parse", "HEAD")
        if base_sha == current_sha:
            self.logger.info(
                "No commits made in fix-up branch, all cookbooks "
                "already at baseline"
            )
            return None
        ...
        if not self.dry_run:
            ...
            if synced and not failed:
                title = f"Fix missing baselines for {len(synced)} cookbook(s)"
            elif synced and failed:
                title = (
                    f"Fix missing baselines for {len(synced)} cookbook(s) "
                    f"({len(failed)} failed)"
                )
            else:
                title = (
                    f"FAILED: Unable to sync {len(failed)} cookbook(s) "
                    f"to baseline"
                )

            body = (
                f"This PR syncs cookbooks that didn't have a baseline "
                f"match during onboarding.\n\n"
                f"**Target baseline:** `{baseline}`\n\n"
            )

            if synced:
                body += "**Successfully synced cookbooks:**\n"
                for cb in synced:
                    body += f"- {cb}\n"
            ...
            if failed:
                ...
                for cb in failed:
                    body += f"- {cb}\n"
                body += (
                    "\nThese cookbooks were not found in upstream at "
                    "the baseline commit. They may be:\n"
                    "- Completely custom (consider adding to "
                    "`ignore_cookbooks`)\n"
                    "- From a different upstream source\n"
            ...
            if existing_pr:
                ...
                run(cmd)
                self.logger.info(f"Fix-up PR #{pr_number} updated")
                return pr_number
            else:
                # Create new PR
                cmd = self._build_gh_pr_command(
                ...
                pr_url = run(cmd)
                pr_number = int(pr_url.strip().split("/")[-1])
                self.logger.info(f"Fix-up PR #{pr_number} created: {pr_url}")
                return pr_number
        else:
            action = "Updated" if existing_pr else "Created"
            self.logger.info(f"[dry-run] {action} fix-up branch {branch}")
            return None
    ...
    def _handle_onboarding_mode(
        self, upstream_config: Dict, prefix: str, trailer_key: str
    ) -> bool:
        """
        Handle onboarding mode when no pointer exists.

        Args:
            upstream_config: Upstream configuration dict
            prefix: Upstream prefix
            trailer_key: Trailer key for this upstream

        Returns:
            True if handled (should return from _sync_upstream)
        """
        self.logger.info(
            f"No upstream pointer found for {prefix}, entering onboarding mode"
        )
        baseline, missing_cookbooks = self.detect_global_baseline(
            upstream_config
        )
        if not baseline:
            self.logger.error(
        ...
        if missing_cookbooks:
            # Reuse the fix-missing-baselines logic
            return self._handle_fix_missing_baselines(
                upstream_config,
                baseline,
                prefix,
                missing_cookbooks=missing_cookbooks,
            )

        # All cookbooks have baselines, proceed with onboarding
        self.create_onboarding_pr(baseline, trailer_key, prefix)
        return True

    def _handle_fix_missing_baselines(
        self,
        upstream_config: Dict,
        baseline_commit: str,
        prefix: str,
        missing_cookbooks: Optional[List[str]] = None,
    ) -> bool:
        """
        Handle missing baselines by creating a fix-up PR.

        Args:
            upstream_config: Upstream configuration dict
            baseline_commit: Commit SHA to use as baseline
            prefix: Upstream prefix
            missing_cookbooks: Optional pre-computed list of missing cookbooks

        Returns:
            True if handled (should return from _sync_upstream)
        """
        if missing_cookbooks is None:
            missing_cookbooks = self.get_cookbooks_missing_baselines(
                upstream_config
            )

        if missing_cookbooks:
            self.logger.warning(
                f"Found {len(missing_cookbooks)} cookbooks without baselines: "
                f"{', '.join(missing_cookbooks)}"
            )

            # Check if fix-up PR already exists
            existing_fixup = self.existing_fixup_pr(prefix)
            if existing_fixup:
                self.logger.info(
                    f"Fix-up PR #{existing_fixup['number']} exists, "
                    f"waiting for merge before onboarding"
                )
                return True

            # Create fix-up PR
            self.logger.info(f"Creating fix-up PR for missing baselines")
            self.create_or_update_fixup_pr(
                missing_cookbooks, baseline_commit, upstream_config
            )
            return True
        else:
            self.logger.info(
                f"No cookbooks with missing baselines for {prefix}"
            )
        return False

    def _process_commits_for_sync(
        self, commits: List[str], upstream_config: Dict
    ) -> Tuple[List[str], bool, str]:
        """
        Process upstream commits, cherry-picking relevant ones.

        Args:
            commits: List of upstream commit SHAs to process
            upstream_config: Upstream configuration dict

        Returns:
            Tuple of (applied_commits, conflict_occurred, sync_branch)
        """
        prefix = upstream_config["prefix"]
        sync_branch = f"{self.pr_branch_prefix}/{prefix}_update"
        ...
        for c in commits:
            ...
            cookbooks_touched = {
                f.split("/")[1]
                for f in files
                if f.startswith(f"cookbooks/{prefix}_") and "/" in f
            }
            self.logger.debug(
            ...
            try:
                ...
                # Extract conflict details from the instance attribute
                conflict_details = getattr(self, "_last_conflict_details", None)
                self.logger.debug(
        ...
        return applied, conflict_occurred, sync_branch

    def _check_for_local_changes_after_sync(
        self, upstream_config: Dict, pointer: str, prefix: str
    ) -> None:
        """
        Check for local changes that need to be pushed upstream.

        Args:
            upstream_config: Upstream configuration dict
            pointer: Current upstream pointer
            prefix: Upstream prefix
        """
        self.logger.info(f"No upstream commits to apply for {prefix}!")
        self.logger.info(f"Checking for remaining local changes for {prefix}")
        all_local_cookbooks = self.list_local_cookbooks(upstream_config)
        cookbooks_with_local_changes = [
            cb for cb in all_local_cookbooks if self.detect_local_changes(cb)
        ]
    ...
    def _push_and_create_sync_pr(
        ...
        """
        Push sync branch and create/update PR.

        Args:
            applied: List of successfully applied commit SHAs
            conflict_occurred: Whether a conflict occurred
            sync_branch: Name of sync branch
            upstream_config: Upstream configuration dict
            trailer_key: Trailer key for this upstream
            prefix: Upstream prefix
        """
    ...
    def _sync_upstream(self, upstream_config: Dict) -> None:
        ...
        # ---------------------------
        if pointer is None:
            self._handle_onboarding_mode(upstream_config, prefix, trailer_key)
            # Regardless of if the onboarding PR was successful or not,
            # we are done with this upstream
            return
        ...
        # Handle --fix-missing-baselines in normal mode
        if self.fix_missing_baselines:
            self._handle_fix_missing_baselines(upstream_config, pointer, prefix)
            # regardless of whether the above worked or not, the user
            # specified this mode, so we do nothing else
            return
        ...
        # Get new commits and process them
        ...
        if commits:
            applied, conflict_occurred, sync_branch = (
                self._process_commits_for_sync(commits, upstream_config)
            )

        # Handle results based on what was applied
        if applied:
            assert (
                sync_branch is not None
            ), "sync_branch must be set if commits were applied"
            self._push_and_create_sync_pr(
                applied,
                conflict_occurred,
                sync_branch,
                upstream_config,
            ...
            self._check_for_local_changes_after_sync(
                upstream_config, pointer, prefix
            )
    ...
    def parse_command(self, body: str) -> Optional[Tuple[str, str]]:
        ...
        cmd_pfx = self.config["bot_command_prefix"]
    ...
    def cmd_split(
        ...
        cmd = self._build_gh_pr_command(
            "view",
            pr_number=pr_number,
            json_fields="number,headRefName,body",
        )
        pr_info = run(cmd)
        ...
        if not self.dry_run:
            ...
            cmd = self._build_gh_pr_command(
                "edit", pr_number=pr_number, add_label=split_label
            )
            run(cmd)
        else:
            self.logger.info(
        ...
        if second_range_upstream:
            new_branch = f"{self.pr_branch_prefix}/{prefix}_{second_range_upstream[0][:8]}"
            self.logger.info(
    ...
    def handle_command(
        ...
        if comment_body is None or pr_number is None:
            if self.github_event_path is None:
                self.logger.error("No event path provided")
                return
            self.logger.debug(
    ...
    def bot_created_pr_or_issue(self, event: dict) -> bool:
    ...
    def bot_created_pr_or_issue_closed(self, event: dict) -> bool:
        ...
        return self.bot_created_pr_or_issue(event)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Line Cook")
    parser.add_argument(
        "--dry-run",
        "-n",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Fix cookbooks without baselines (auto during bootstrap)",
    )
    parser.add_argument(
        "-l",
        "--log-level",
    ...
    parser.add_argument(
        "--base-branch",
        help="Base branch for PRs (default: from config or 'main')",
    )
    parser.add_argument(
        "--pr-branch-prefix",
        help="Prefix for PR branches (default: from config or 'line-cook')",
    )
    parser.add_argument(
        "--target-remote",
        default="origin",
    ...
    bot = LineCook(
        ...
        fix_missing_baselines=args.fix_missing_baselines,
    ...
    if args.command and args.pr:
        ...
        bot.handle_command()
    elif github_event_name in ("issues", "pull_request_target") and event:
    ...
    elif github_event_name == "pull_request" and event:
        if bot.bot_created_pr_or_issue(event):
            logger.info("Ignoring update on PR we created")
        else:
            bot.sync()
